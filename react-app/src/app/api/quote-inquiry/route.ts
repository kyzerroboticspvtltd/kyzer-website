...
export async function POST(req: NextRequest) {
  const rl = await rateLimit(`quote-inquiry:${getIp(req)}`, 5, 60 * 60_000);
  ...
  try {
    ...
    // 1. Save to Supabase (primary â€” admin visibility)
    ...
    // 2. Send emails (non-fatal â€” don't fail the request if email is unconfigured)
    ...
    const html = `
      <div style="font-family:sans-serif;max-width:620px;margin:auto;">
        ...
        <p style="color:#888;font-size:13px;margin-bottom:24px;">Submitted via kyzerrobotics.com Â· ${esc(inquiryId)}</p>
        ...
        <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
          <tr><td style="padding:8px 12px;color:#888;width:130px;">Type</td><td style="padding:8px 12px;font-weight:600;">${esc(projectType) || 'â€”'}</td></tr>
          <tr style="background:#f9f9f9;"><td style="padding:8px 12px;color:#888;">Timeline</td><td style="padding:8px 12px;">${esc(timeline) || 'â€”'}</td></tr>
          <tr><td style="padding:8px 12px;color:#888;">Budget</td><td style="padding:8px 12px;">${esc(budget) || 'â€”'}</td></tr>
        </table>
        ...
        <p style="font-size:12px;color:#bbb;">Kyzer Robotics Â· Quote Inquiry Form</p>
      </div>`;

    try {
      await sendMail({ to: NOTIFY_EMAIL(), subject: `[Kyzer Quote] ${projectType ? esc(projectType) + ' â€” ' : ''}${esc(name)}`, html });
      const waPhone = NOTIFY_PHONE();
      const waLink = waPhone ? `https://wa.me/${waPhone.replace(/\D/g, '')}` : '#';
      await sendMail({
        to: email,
        subject: 'We received your project request â€” Kyzer Robotics',
        html: `
          <div style="font-family:sans-serif;max-width:600px;margin:auto;">
            ...
            <a href="${waLink}" style="display:inline-block;margin:12px 0;background:#25D366;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;">Chat on WhatsApp â†’</a>
            <hr style="border:none;border-top:1px solid #eee;margin:24px 0;">
            <p style="font-size:13px;color:#888;">Kyzer Robotics Â· Pune, Maharashtra<br>${esc(NOTIFY_EMAIL())}</p>
          </div>`,
      });
    } catch {
      // Email failed â€” inquiry is already saved to Supabase, so return success
    }
