...
export default function DashboardPage() {
  ...
  if (loading) {
    return (
      <div style={{ minHeight: '100vh', background: '#f9f9f7', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
        <style>{`@keyframes db-pulse{0%,100%{opacity:1}50%{opacity:0.4}} .db-logo{animation:db-pulse 1.4s ease-in-out infinite;}`}</style>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo.png" alt="Kyzer Robotics" className="db-logo" style={{ width: 80, height: 'auto' }} />
        <span style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: '#bbb', letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 500 }}>Loading orders…</span>
      </div>
    );
  }
