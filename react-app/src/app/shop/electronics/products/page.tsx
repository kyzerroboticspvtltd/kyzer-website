...
export default function ElectronicsProductsPage() {
  ...
  function svgPlaceholder(label: string) {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="400" height="200"><rect width="100%" height="100%" fill="%23f2f0eb"/><text x="50%" y="44%" font-family="sans-serif" font-size="32" fill="%23cccccc" text-anchor="middle">⚡</text><text x="50%" y="68%" font-family="sans-serif" font-size="13" fill="%23bbbbbb" text-anchor="middle">${encodeURIComponent(label)}</text></svg>`;
              ...
              return (
                <div key={p.id} onClick={() => setSelected(p)}
                  ...
                  <div style={{ height: 170, background: '#f4f4f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 56, overflow: 'hidden' }}>
                    <img src={photo || svgPlaceholder(subcatLabel[p.subcat] || 'Electronics')} alt="" style={{ width: '100%', height: '100%', objectFit: photo ? 'contain' : 'cover', padding: photo ? 16 : 0 }} />
                  </div>
