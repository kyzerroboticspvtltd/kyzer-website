...
export function ProductCard({ product, onAddToCart }: ProductCardProps) {
  ...
  return (
    <Card className="group relative flex flex-col overflow-hidden border border-black/[0.07] bg-white rounded-2xl transition-all duration-200 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:border-[#FF8C35]/30">
      {/* Image / Emoji area */}
      <div className="relative flex items-center justify-center h-44 bg-[#f4f4f2] border-b border-black/[0.06] overflow-hidden transition-colors duration-300 group-hover:bg-[#fff5ed]">
        {product.image ? (
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        <div className="flex-1">
          ...
          <p className="text-[13px] text-gray-500 leading-relaxed line-clamp-2">{product.shortDesc}</p>
        </div>
        ...
        <div className="flex items-center justify-between gap-2 pt-2 border-t border-black/[0.06]">
          ...
          <div className="flex gap-1.5">
            <Button
              ...
              onClick={() => window.open(`https://wa.me/919049695264?text=${encodeURIComponent(whatsappText)}`, '_blank')}
            >
